...
|-----------------------------------------------+-------------------------------|
| Solver code on github by /2012 Burns et. al./ | [ /h/, LIFO (*undocumented*)] |
...
(Before proceeding, let me say...)
#+END_SMALLER

Zero-cost enables the modelling of many *practical applications.*
...
+ *Cybersec is also an application domain.* (Boddy et. al.)
  ...
  #+END_ALIGNRIGHT
+ Consider a *REALISTIC transportation domain w/ fuel minimization.*
  #+BEGIN_ALIGNRIGHT
...
Ignore the case h＞0 (because 0≦h≦h^*=0)
...
+ which represents a *realistic key resource*. For example,
  ...
  + Consume =fuel= in Driverslog (transportation domain)
  + etc..
...
+ *Randomized depth-based tiebreaking outperforming the traditional strategies*
  + Effective in the Final Frontier of the zero-cost domains, where no heuristics can help
  + Depth-based approach has little disadvantage in positive-cost domains (depth is always 0)
+ *Understanding of the structure of the final plateau in zero-cost domains*
+ *Depth Bias is harmful -- classic exploration vs exploitation problem* in optimising search
  + Various applications and connections to the other areas
  + Sequential Satisficing Search ( cf. Yesterday talk by Richard Valenzano! )
    + Use f/5 and ignore remainder : constant-error search /＜f^*+5/ solution?
    + ↔ WA* with weight /w/ only provides /＜w×f^*/ solution
  + Temporal Planning/Scheduling
    + Short interleaved actions no longer increase the total makespan = 0-cost actions?
  + Can we make the exploration more efficient?
    + RRTs (Rapidly exploring Random Tree), MCTS (Monte Carlo Tree Search)
    + Diverse BFS (Imai AAAI11), Plan-distance (RPGoldman AAAI15) etc...
+ "Plan Diversity is underinvestigated" --- Jorg Hoffman
  + Related "exploration-based approach" might also be underinvestigated in planning?

#+END_SMALLER

    # + Desparate wisdom: *Does planning algorithms extend to continuous search?*
#+BEGIN_CENTER
