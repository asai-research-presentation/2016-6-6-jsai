...
Research on Tiebreaking is mostly forgotten.
...
|                                | Tiebreaking                                         |
|--------------------------------+-----------------------------------------------------|
| Current rule-of-thumb          | select *least h cost* (*h-based tiebreaking*)       |
|--------------------------------+-----------------------------------------------------|
| 1968 Hart, Nilsson and Raphael | *least h*, then *arbitrarily* -- *Folklore*?        |
| 1985 Korf, AIJ                 | *most recently generated* (LIFO) -- forgotten?      |
|--------------------------------+-----------------------------------------------------|
| 2007 Hansen and Zhou           | *least h* because it's "well-known" to perform good |
| 2010 Holte                     | *least h* because it's "most often done"            |
| 2012 Burns et. al.             | *least h* (+ LIFO, undocumented)                    |
| Fast Downward (current)        | *least h* (+ FIFO, undocumented)                    |
|--------------------------------+-----------------------------------------------------|
...
#+BEGIN_RESUME
The research on tiebreaking strategy of A* is mostly forgotten.
...
and could be seen as one of the oldest folklore in the heuristic search literature.

There are also another tiebreaking method called LIFO order, which was already common in 1985.
LIFO claims that the nodes should be expanded in most-recently-generated-first.

Recently, we rarely find a detailed description on these strategies.
It is sometimes said: it's common or it's known to perform well.
There are sometimes no mention.
...
if multiple nodes have the same f value, it selects the smallest h.
If even h are the same, it breaks ties according to FIFO.
...
* The Unfriendly Effect on h-based Tiebreaking
...
On *Openstacks* & *Cybersec*,

/h/-tiebreaking has no effect,

i.e.

the search is controlled entirely by LIFO and FIFO !
...
In these domains, h-tiebreaking has almost no effect, and the LIFO and FIFO plays the primary role. 
#+END_RESUME
...
#+BEGIN_RESUME
We want to further improve the performance in such zero-cost domains.
For this purpose, we should understand what's happening in the final plateau.

Firstly, in most domains, including positive-cost domains, most nodes are in f=f*.
Without proper tiebreaking strategy, the planner has no guidance,
because any heuristic estimates are the same within this region.
#+END_RESUME
